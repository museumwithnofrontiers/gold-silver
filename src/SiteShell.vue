...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('goldSilver.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('goldSilver.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('goldSilver.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
